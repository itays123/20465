...
- There are no nested macro definitions
- Macro definition must end */
static int mpass_process_definition(FILE *, char *, table *, char *, char *);
...
static int mpass_process_line(FILE *source, char *line, table *macros, FILE *out)
{
    ...
    if (str_equal(opstart, opend, "macro"))
    {
        ...
        mpass_process_definition(source, line, macros, keystart, keyend); /* Assumption: Macro definition WILL end */
    }

    if ((existng_definition = find_item(macros, opstart, opend)) != NULL)
        mpass_process_reference(source, line, existng_definition, out);
    
    /* Not a macro definition nor a macro reference */
    fputs(line, out);
    return 0;
}
